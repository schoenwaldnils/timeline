...
class Timeline extends Component {
  ...
  static getDerivedStateFromProps(nextProps) {
    ...
    const { timesHeight, times: positionedTimes } =
      sortedTimes && positionTimes(sortedTimes)
    const { eventsHeight, events: positionedEvents } =
      sortedEvents && positionEvents(sortedEvents)
  ...
  render() {
    const { activeElement, changeSidebarContent } = this.props

    const { times, timesHeight, events, eventsHeight } = this.state

    const scaleNumberNegativ = []
    for (let i = 0; i <= 41; i += 1) {
      scaleNumberNegativ.push(
        <div className="Timeline-scaleNumber" key={i}>
          {i * -100}
        </div>,
      )
    }
    ...
    return (
      <div className="Timeline">
        ...
        <div className="Timeline-numbers">
          <div className="Timeline-negativeNumbers">{scaleNumberNegativ}</div>
          <div className="Timeline-positiveNumbers">{scaleNumberPositive}</div>
        </div>
